...
# Create default SSL context for all HTTPS requests
default_ssl_context = setup_ssl_context()
print (default_ssl_context)
# Make sure EVERY urlopen in this process uses our context
_opener = urllib.request.build_opener(
    urllib.request.HTTPSHandler(context=default_ssl_context)
)
urllib.request.install_opener(_opener)  # <-- important

# Also point Python/OpenSSL and requests to the certifi bundle
os.environ["SSL_CERT_FILE"] = certifi.where()
os.environ["REQUESTS_CA_BUNDLE"] = certifi.where()
# Optional: torch.hub honors CURL_CA_BUNDLE on some setups
...
class JewelryMobileNetV3(nn.Module):
    def __init__(self, num_classes=1000):
        ...
        try:
            ...
            # Fallback so app still runs; embeddings won't be great until weights are present
...
class FastImageIndexer:
    ...
    def create(self, folder_path):
        ...
        print(f"Index build complete. Inserted: {inserted}, Skipped: {skipped}")
        return len(self.image_paths)
    ...
    def search(self, query_image_path, k=5, threshold=0.8):
        ...
        # guard against all-zero (or extremely small) similarities
        max_similarity = float(np.max(distances[0])) if len(distances[0]) else 0.0
        print (max_similarity)
        if not np.isfinite(max_similarity) or max_similarity <= 1e-8:
            return []
        ...
        for idx, dist in zip(indices[0], distances[0]):
            if idx < 0:
                continue
            normalized_similarity = float(dist) / max_similarity
